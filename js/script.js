...
let chargeCurrent = $('.charge_current').val();;
let coordXY = [];
let coordXYWeek = [];
const input = document.querySelector('.consumption');
...
$('.charge_current').change(function () {
    equel ();
});
...
$('.power_truck').change(function () {
    if ($('.power_truck').val() === "custom") {
        ...
        $('.consumption').val($('.power_truck').val());
        $('.consumption').attr('disabled', 'disabled');
        $('.consumption').css({
...
function creatYCoord(coordXY, coordXYWeek, bufArrayXY) {
    ...
    const batteryVoltage = $('.battery_voltage').val();
    const truckPower = $('.consumption').val();
